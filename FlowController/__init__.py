...
# import threading
# import xmlrpc
# from SimpleMessageQueue import SMQServer, SMQClient, SimpleXMLRPCServerEx
# import pandas as pd
# import subprocess
# 
...
#         return os.path.abspath(os.path.join(self._joblog_dir,
...
#         cfg['logo'] = None
#         try:
#             logo_filename = os.path.join(os.path.dirname(cfg_filename), cfg['logo_filename'])
...
#                 raise Exception(f'Error!  The lockfile {lock_filename} exists so another instance may be already ' +
#                                 'running.  If you are sure there is no other instance running, please delete the ' +
#                                 'lockfile to continue.')
# 
#             # lock
...
#                 s = f.read()
#                 if str(os.getpid()) != s:
#                     raise Exception(f'Error!  The lockfile {lock_filename} pid of {s} does not match pid ' +
#                                     f'of {os.getpid()}')
#         except Exception as _:
#             logging.error(traceback.format_exc())
#             exit(1)
...
#         self._smqc.start_client(smq_server_url, self._smq_uid, ' '.join(SMQ_PUBLIST), ' '.join(SMQ_SUBLIST),
...
# #         smqc = SMQClient()
# #         smqc.start_client(smq_server_addr, 'FlowController_Discovery', 'FlowController.CQ',
# #                           'FlowController.CQ_Response')
# # 
# #         # send a ping message
